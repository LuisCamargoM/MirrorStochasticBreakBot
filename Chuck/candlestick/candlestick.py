...
__builders = dict()
__default_ohlc = ['open', 'max', 'min', 'close']
...
def candle_force(candles_df,
                   ohlc=__default_ohlc,
                   is_reversed=False,
                   target=None):
    candl_force = __create_object('CandleForce', target)
    return candl_force.has_pattern(candles_df, ohlc, is_reversed)
